...
sidebar: sidebar 
keywords:  
summary: 了解 StorageGRID 12.1.0 中已删除或弃用的特性和功能。在升级之前，请检查任何已删除或已弃用的功能是否会影响客户端应用程序或网格配置。 
---
= StorageGRID 中已删除或弃用的功能和特性
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
了解 StorageGRID 12.1.0 中已删除或弃用的特性和功能。在升级之前，请检查任何已删除或已弃用的功能是否会影响客户端应用程序或网格配置。
...
已弃用:: 您*不应*在新的生产环境中使用此功能。现有生产环境可以继续使用此功能。
...
12.3
 a| 
从 StorageGRID 12.1.0 开始，已弃用对新存储的静态对象数据的 AES-128 加密支持。继续支持检索最初使用 AES-128 加密的存储对象。
...
. 停用所有存档节点。请参阅 https://docs.netapp.com/us-en/storagegrid-118/maintain/grid-node-decommissioning.html["网格节点停用(StorageGRID 11.8文档站点)"^]。
. 从存储池和 ILM 策略中删除所有归档节点引用。请参阅 https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_11.9_software_upgrade_resolution_guide["NetApp知识库：《StorageGRID软件升级解决方案指南》"^]。
...
12.1
 a| 
https://docs.netapp.com/us-en/storagegrid-118/monitor/managing-alarms.html#configure-notifications-for-alarms-legacy-system["为警报配置通知（旧系统）（StorageGRID 11.8）"^]
