...
summary: StorageGRID 使用的 S3 REST API 会定期更新。 
---
= 对 StorageGRID S3 REST API 支持的更改
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
StorageGRID 使用的 S3 REST API 会定期更新。
...
* `If-Match`
+
根据您的 StorageGRID 配置， `If-Match` 标头的行为会有所不同。提供此标头可能会返回错误消息（501 Not Implemented）。如果 `If-Match` 标头被接受但不起作用，则可能会触发警报，其中包含有关启用更严格的 501 Not Implemented 错误行为的说明。

* `If-None-Match`
+
根据您的 StorageGRID 配置， `If-None-Match` 标头的行为会有所不同。提供此标头可能会返回错误消息（501 Not Implemented）。如果 `If-None-Match` 标头被接受但不起作用，则可能会触发警报，其中包含有关启用更严格的 501 Not Implemented 错误行为的说明。

* 为以下请求和支持的标头添加了对预先计算的校验和值的支持：
+
** CompleteMultipartUpload: `x-amz-checksum-crc32`、 `x-amz-checksum-crc32c`和 `x-amz-checksum-crc64nvme`
** CreateMultipartUpload: `CRC32`, `CRC32C`,以及 `CRC64NVME`用于 `x-amz-checksum-algorithm`
** CopyObject: `CRC32`, `CRC32C`, `CRC64NVME`,以及 `SHA256`用于 `x-amz-checksum-algorithm`
...
* 添加了对 `x-amz-checksum-type`的支持，适用于 CreateMultipartUpload 和 CompleteMultipartUpload。
* 增加了对 `s3:x-amz-copy-source`策略条件键的支持。
* 允许在现有存储桶上启用 S3 对象锁定。
...
* 添加了对 STS AssumeRole 和会话策略的支持。请参阅link:../s3/example-session-policies.html["会话策略示例"]。您可以在租户组下设置 AssumeRole。
...
11.7
 a| 
* 已添加 link:../s3/quick-reference-support-for-aws-apis.html["快速参考：支持的S3 API请求"]。
...
|===
有关详细信息，请参见 link:../s3/index.html#supported-versions["S3 REST API 支持的版本和更新"]。
