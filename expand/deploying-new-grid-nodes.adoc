...
.步骤
. link:../swnodes/deploying-storagegrid-node-as-virtual-machine.html["将新节点部署为虚拟机"]并将其连接到一个或多个StorageGRID网络。
+
...
.开始之前
* 您已获得针对您的 Linux 版本安装StorageGRID 的说明，并且您已查看link:../swnodes/index.html["硬件和存储要求"]。
...
. 如果需要手动访问监视器安装页面，请从菜单栏中选择*Monitor Installation*。
+
监视器安装页面显示安装进度。
