...
S3 客户端发出请求 (SUPD)，以更新 S3 对象 (S3KY) 的指定元数据(`x-amz-meta-*`。

在此示例中，请求标头包含在 HTRH 字段中，因为它已配置为审核协议标头（*Configuration* > *Monitoring* > *Audit and syslog server*）。请参阅 link:../monitor/configure-log-management.html["配置日志管理和外部系统日志服务器"]。
