...
summary: 当 S3 客户端发出 HEAD 操作时，会发出请求来检查对象或存储桶是否存在并检索有关对象的元数据。如果操作成功，服务器将发出此消息。 
---
...
[role="lead"]
当 S3 客户端发出 HEAD 操作时，会发出请求来检查对象或存储桶是否存在并检索有关对象的元数据。如果操作成功，服务器将发出此消息。
...
仅当 S3 请求使用来自 AssumeRole 请求的临时安全凭据时才显示。
