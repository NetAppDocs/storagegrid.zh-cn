...
* 如果要允许 S3 租户使用网格联合连接同步帐户数据并将存储区对象复制到另一个网格：
+
...
** 使此租户帐户以及所有租户组、用户和 S3 访问密钥在此网格与选定连接中的其他网格之间同步。
+

TIP: 如果出于安全或合规原因需要，您可以取消选择*启用访问密钥同步*。
...
. 如果您为租户选择了*使用网格联合连接*权限，请确认已将相同的租户复制到连接中的另一个网格。两个网格上的租户将具有相同的 20 位帐户 ID、名称、描述、配额和权限。
+

NOTE: 如果您看到一条错误消息，指出租户帐户是在此网格上创建的，但未同步到其他网格，请参阅 link:grid-federation-troubleshoot.html["对网格联合错误进行故障排除"] 中的说明。
...
按照 link:../tenant/index.html["使用租户帐户"] 中的说明管理租户组和用户、S3 访问密钥、存储桶、平台服务以及帐户同步和跨网格复制。
