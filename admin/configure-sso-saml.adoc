...
summary: '您可以按照针对 SAML 的配置 SSO 向导并进入沙盒模式来配置和测试单点登录 (SSO)，然后再为所有 StorageGRID 用户启用它。启用 SSO 后，您可以在需要更改或重新测试配置时返回沙盒模式。' 
---
= 使用 SAML 在 StorageGRID 中配置 SSO
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
按照为 SAML 配置 SSO 向导并进入沙盒模式来配置和测试单点登录 (SSO)，然后再为所有 StorageGRID 用户启用它。启用 SSO 后，您可以在需要更改或重新测试配置时返回沙盒模式。
...
要允许 StorageGRID （服务提供商）和 SSO 身份提供商安全地就用户身份验证请求进行通信，请完成以下任务：

. 在StorageGRID中配置设置。
. 使用 SSO 标识提供程序的软件为每个管理节点创建依赖方信任 (AD FS)、企业应用程序 (Entra ID) 或服务提供程序 (PingFederate)。
...
== [[enter-sandbox-mode]]步骤 2：提供依赖方信任、企业应用程序或 SP 连接

根据您选择的 SSO 类型，填写向导第 2 步中的字段。
...
. 按照link:../admin/creating-enterprise-application-entra-id-saml.html["在 Entra ID 中创建企业应用程序"]中的步骤为表中列出的每个管理节点创建企业应用程序。
...
.PingFederate
--
填写 *Provide SP connection ID* 步骤中的字段。
...
+
有关说明，请转到 link:../admin/creating-relying-party-trusts-in-ad-fs-saml.html["在 AD FS 中创建依赖方信任"]。
...
. 转到 Azure 门户。
. 按照 link:../admin/creating-enterprise-application-entra-id-saml.html["在 Entra ID 中创建企业应用程序"] 中的步骤，将每个管理节点的 SAML 元数据文件上传到其相应的 Entra ID 企业应用程序中。
...
. 转到 PingFederate 。
. link:../admin/creating-sp-connection-ping-saml.html["为 StorageGRID 创建一个或多个服务提供商（ SP ）连接"]。使用每个管理节点的 SP 连接 ID（显示在配置 SSO 页面上的表格中）以及为该管理节点下载的 SAML 元数据。
+
...
.下一步是什么？
使用 SAML 配置 SSO 后，为管理节点配置身份提供程序。
...
Active directory
 a| 
link:creating-relying-party-trusts-in-ad-fs-saml.html["为每个管理节点创建应用程序组"]
...
进入 ID
 a| 
link:creating-enterprise-application-entra-id-saml.html["为每个管理节点创建应用程序"]
...
PingFederate
 a| 
link:creating-sp-connection-ping-saml.html["为每个管理节点创建服务提供商连接"]
