...
summary: 跨网格复制是在网格联合连接中连接的两个 StorageGRID 系统中所选 S3 存储桶之间的对象自动复制。跨网格复制需要帐户同步。 
---
...
[role="lead"]
跨网格复制是在连接于 link:grid-federation-overview.html["网格联合连接"] 的两个 StorageGRID 系统中所选 S3 存储桶之间自动复制对象的功能。link:grid-federation-what-is-account-sync.html["帐户同步"] 是跨网格复制所必需的。
...
image:grid_federation_cgr_workflow.png["跨网格复制工作流"]
...
如果租户帐户具有*使用网格联合连接*权限以使用一个或多个 link:grid-federation-overview.html["网格联合连接"]，则具有 Root 访问权限的租户用户可以在每个网格上的相应租户帐户中创建存储桶（"源"和"目标"存储桶）。这些存储桶：
...
创建两个存储桶后，您可以为其中一个或两个存储桶配置跨网格复制。
...
image:grid_federation_cross_grid_replication_one_direction.png["显示一个方向的网格联合连接的图像"]
...
image:grid_federation_cross_grid_replication.png["显示单向复制与双向复制的图像"]
...
image:grid_federation_cross_grid_replication_delete.png["显示在两个网格上复制客户端删除的图像"]
...
CAUTION: 您可以在现有存储桶上启用 S3 Object Lock，这可能会导致稍后出现配置错误。


NOTE: 如果在源存储桶上启用了 S3 对象锁定，请确保在目标存储桶上也启用了它。如果在目标存储桶上禁用了 S3 对象锁定，则在复制过程中会发生错误。请参阅 link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors["跨网格复制警报和错误"]。
