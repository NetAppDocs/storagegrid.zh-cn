...
image:metadata_space_across_sites.png["站点之间的元数据空间"]
...
新 StorageGRID 12.1 或更高版本安装的默认设置（其中每个存储节点具有 128 GB 或更多 RAM）为 20,000 GB（20 TB）。
...
+
image:storage_used_object_metadata_actual_reserved.png["已用存储 - 对象元数据 - 实际预留"]



在屏幕截图中，实际保留值为 20 TB。此屏幕截图适用于新 StorageGRID 12.1 安装中的大型存储节点。卷 0 至少为 20 TB，因此它使用默认的系统范围元数据保留空间设置。
...
假设您使用 12.1 版或更高版本安装新的 StorageGRID 系统。在此示例中，假设每个存储节点的 RAM 超过 128 GB，并且存储节点 1 (SN1) 的卷 0 为 6 TB。基于这些值：

* 全系统*元数据保留空间*设置为 20 TB。（如果每个存储节点的 RAM 超过 128 GB，这是新 StorageGRID 12.1 或更高版本安装的默认值。）
* SN1 元数据的*实际保留*空间为 6 TB。（整个卷被保留，因为卷 0 小于*元数据保留空间*设置。）
...
image:metadata_allowed_space_volume_0.png["元数据允许的空间卷 0"]
...
&lt ； = 4 TB
 a| 
元数据实际预留空间的 75%，最高可达 1.32 TB
 a| 
元数据实际保留空间的 75%，最多 1.98 TB
...
管理； 4 TB
 a| 
（元数据的实际预留空间 − 1 TB）× 75%，最多 1.32 TB
 a| 
（元数据的实际预留空间 − 1 TB）× 75%，最多 7.92 TB
...
+
image:storage_used_object_metadata_allowed.png["已用存储 - 对象元数据 - 允许"]



在屏幕截图中，*允许*值为 7.92 TB，这是元数据实际保留空间超过 4 TB 的 Storage Node 的最大值。
...
* 全系统*元数据保留空间*设置为 20 TB。（如果每个存储节点的 RAM 超过 128 GB，这是新 StorageGRID 12.1 或更高版本安装的默认值。）
* SN1 元数据的*实际保留*空间为 6 TB。（整个卷被保留，因为卷 0 小于*元数据保留空间*设置。）
* SN1 上元数据的*允许*空间为 3.75 TB，基于<<table-allowed-space-for-metadata,元数据允许的空间表>>中所示的计算：（元数据的实际保留空间 − 1 TB）× 75%，最多 7.92 TB。
...
image:metadata_space_three_storage_nodes.png["元数据空间三个存储节点"]
