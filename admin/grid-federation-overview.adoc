...
summary: 您可以使用网格联合在多个 StorageGRID 系统之间同步租户并复制其对象以进行灾难恢复。 
---
= 了解 StorageGRID 网格联合
:allow-uri-read: 
:icons: font
...
[role="lead"]
您可以使用网格联合在多个 StorageGRID 系统之间同步租户并复制其对象以进行灾难恢复。
...
image:grid_federation_workflow.png["网格联合连接工作流"]
...
+
有关版本要求的详细信息，请参见 https://library.netapp.com/ecm/ecm_download_file/ECMLP3373065["发行说明"^]。您必须使用您的 NetApp 帐户登录或创建帐户才能访问发行说明。
...
* 必须至少存在一个网格联盟连接，然后才能使用 link:grid-federation-what-is-account-sync.html["帐户同步"] 或 link:grid-federation-what-is-cross-grid-replication.html["跨网格复制"]。
...
image:grid_federation_certificates.png["网格联合证书"]
