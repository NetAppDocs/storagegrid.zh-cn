...
NOTE: 尽管StorageGRID负载均衡器服务是推荐的负载均衡机制，但您可能希望集成第三方负载均衡器。如需了解更多信息，请联系您的NetApp客户代表或参考以下内容： https://docs.netapp.com/us-en/storagegrid-enable/technical-reports/load-balancer-index.html["将第三方负载均衡器与StorageGRID结合使用"^]。
...
如果您计划使用 S3 虚拟托管样式请求，则证书还必须为您配置的每个 link:../admin/configuring-s3-api-endpoint-domain-names.html["S3端点域名"] 包含一个*备用名称*条目，包括任何通配符名称。例如：
