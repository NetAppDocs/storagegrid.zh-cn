...
---
= 在 StorageGRID 中为 SAML 创建 Entra ID 企业应用程序
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用 Entra ID 为系统中每个用于 SAML 的管理节点创建企业应用程序。

.开始之前
* 您已开始为StorageGRID配置单点登录，并选择 *Entra ID* 作为 SSO 类型。
* 您在 Grid Manager 中拥有 link:../admin/configure-sso-saml.html#enter-sandbox-mode["进入沙盒模式"]。
...
== 步骤2：创建企业应用程序并保存 StorageGRID SSO 配置
...
. link:../admin/configure-sso-saml.html#test-sso["测试每个应用程序"]。
