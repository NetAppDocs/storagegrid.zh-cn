...
summary: 如果要同步租户详细信息并复制对象数据，可以在 StorageGRID 系统之间创建网格联合连接。 
---
...
[role="lead"]
如果要同步租户详细信息并复制对象数据，可以在 StorageGRID 系统之间创建网格联合连接。

如图所示、创建网格联合连接包括两个网格上的步骤。您可以在一个网格上添加连接、并在另一个网格上完成连接。您可以从任一网格开始。


TIP: 或者，您也可以 link:../admin/grid-federation-automate-connections.html["自动创建、编辑和维护网格联合连接"]。

image:grid_federation_create_connection.png["在两个网格上创建连接的步骤的图形概述"]
...
. 选择*配置* > *系统* > *网格联合*。
. 选择*添加连接*。
. 输入连接名称，这是一个唯一的名称，可帮助您识别此连接。例如， `Grid 1-Grid 2`。
. 如果需要link:../admin/configure-global-namespace.html["配置全局命名空间"]，请选择*Enable global namespace*。
. 选择 * 继续 * 。
. 输入网格详细信息，如下：
...
* link:grid-federation-troubleshoot.html["排除连接故障"]，包括解决与帐户同步和跨网格复制相关的任何警报和错误。
