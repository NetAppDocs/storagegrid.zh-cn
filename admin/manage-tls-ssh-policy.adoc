...
+
image:securitysettings_tls_ssh_policies_current.png["TLS和SSH策略"]
...
.FIPS严格
--
如果您需要通用标准认证，并且需要使用 NetApp 加密安全模块 (NCSM) 3.1.2 或 NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 模块进行与负载平衡器端点、租户管理器和网格管理器的外部客户端连接，请使用 FIPS 严格策略。使用此策略可能会降低性能。

NCSM 3.1.2 和 NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 模块用于以下操作：
...
+
有关详细信息，请参见 NIST Cryptographic Algorithm Validation Program link:https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/details?validation=40426["证书 #A7816"^]。
...
+
有关详细信息，请参见 NIST Cryptographic Algorithm Validation Program link:https://csrc.nist.gov/projects/cryptographic-module-validation-program/entropy-validations/certificate/223["熵证书#E223"^]。
...
或者，如果您的 StorageGRID 具有 FIPS 140 加密要求，则启用 FIPS 模式功能以使用 NCSM 3.1.2 和 NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 模块：
...
. 选择*复制到剪贴板*，然后选择*取消*。
+
image:securitysettings_custom_security_policy_copy.png["复制现有策略以创建自定义策略"]
