...
. 选择*重试帐户克隆*。
+
image:grid-federation-retry-account-clone.png["显示错误消息和重试帐户克隆按钮的屏幕截图"] 如果已解决此错误，租户帐户现在将克隆到另一个网格。
...
image:grid-federation-last-error.png["屏幕截图、显示网格联盟连接的最后一个错误列中的消息"]
