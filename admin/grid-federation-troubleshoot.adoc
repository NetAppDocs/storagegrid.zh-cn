...
summary: 您可能需要排除与网格联合连接、帐户同步和跨网格复制相关的警报和错误。 
---
...
[role="lead"]
您可能需要排除与网格联合连接、帐户同步和跨网格复制相关的警报和错误。
...
.问题描述
无法登录到已同步的租户帐户。租户管理器登录页面上的错误消息是"此帐户的凭据无效。请重试。"

.详细信息
为租户的本地 root 用户和其他本地用户设置的密码将自动同步。但是，在登录到目标网格上的租户帐户之前，您可能需要更改目标网格上租户的本地 root 用户的密码。
...
.问题描述
在创建具有*使用网格联合连接*权限的新租户后，您会看到消息"Tenant created without a sync"。
...
. 找到无法同步的租户帐户。
. 选择租户名称以显示详细信息页面。
. 选择*重试同步*。如果错误已解决，租户帐户现在将同步到另一个网格。
...
image:grid_federation_last_error.png["屏幕截图、显示网格联盟连接的最后一个错误列中的消息"]
...
. 执行任何建议的操作。
+
例如，如果版本控制在目标存储桶上暂停以进行跨网格复制，请重新启用该存储桶的版本控制。或者，如果在跨网格复制的目标存储桶上禁用了 S3 对象锁定，则为该存储桶启用 S3 对象锁定。
