...
[role="lead"]
您可以通过创建或导入组和用户并为每个组分配权限来控制谁可以访问 StorageGRID 以及用户可以执行哪些任务。也可以启用单点登录 (SSO)、创建客户端证书、更改网格密码以及使用多管理员验证。
...
StorageGRID 系统支持使用安全断言标记语言 2.0 (SAML 2.0) 或 OpenID Connect (OIDC) 标准的单点登录 (SSO)。在您link:how-sso-works.html["配置并启用SSO"]之后，所有用户都必须通过外部身份提供程序的身份验证，然后才能访问 Grid Manager、Tenant Manager、网格管理 API 或租户管理 API。本地用户无法登录 StorageGRID。
...
您可以使用多管理员验证 (MAV) 来确保只有在指定管理员批准后才能执行某些 StorageGRID 操作。MAV 可防止受损、恶意或缺乏经验的管理员进行不必要的更改或删除数据。
