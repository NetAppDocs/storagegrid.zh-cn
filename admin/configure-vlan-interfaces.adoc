...
summary: '您可以在管理节点和网关节点上创建虚拟 LAN (VLAN) 接口，并在 HA 组和负载平衡器端点中使用它们，以隔离和分区流量，从而实现安全性、灵活性和性能。' 
---
...
+
image:vlan_create_parent_interfaces.png["VLAN 创建向导的父接口"]
...
** 如果需要删除父接口，请选择垃圾桶 image:icon_trash_can.png["垃圾桶图标"]。
