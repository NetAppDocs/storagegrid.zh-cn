...
---
= StorageGRID 中的 S3 CreateMultipartUpload 请求标头和选项
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: 如果 CreateMultipartUpload 请求未指定校验和，StorageGRID 不会计算对象的默认 CRC-64/NVME 校验和。StorageGRID 忽略无效或不受支持的 `x-amz-checksum-algorithm`值。
