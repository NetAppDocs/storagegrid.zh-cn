...
StorageGRID 不解析或解释用户定义元数据的键名或值中包含的转义 UTF-8 字符。转义 UTF-8 字符被视为 ASCII 字符：

* 如果用户定义的元数据包含转义的 UTF-8 字符，则请求将成功。
* 如果键名称或值的解释值包含不可打印的字符，StorageGRID 不会返回 `x-amz-missing-meta`标头。
...
+
复制对象时，如果源对象具有 `checksum`或包含 `x-amz-checksum-algorithm`，则复制的对象具有 FULL_OBJECT 校验和。

+
如果源对象具有校验和，但请求不包括 `x-amz-checksum-algorithm`或它使用不受支持的校验和值，则复制的对象与源对象具有相同类型的校验和。StorageGRID 忽略无效或不受支持的 `x-amz-checksum-algorithm`值。

+
如果源对象没有校验和，并且没有包含 `x-amz-checksum-algorithm`或使用不受支持的校验和值，则复制对象没有校验和。

+

NOTE: 如果 CopyObject 请求未指定校验和，并且源对象没有校验和，则 StorageGRID 不会计算对象的默认 CRC-64/NVME 校验和。
...
* 未使用匹配的 ILM 规则中指定的导入行为选项。当正常后台 ILM 流程重新评估 ILM 时，将对更新触发的对象放置进行任何更改。
+
