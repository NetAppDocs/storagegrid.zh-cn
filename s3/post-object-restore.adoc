...
---
= 使用 S3 RestoreObject 请求在 StorageGRID 中还原对象
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
*重要提示*：如果对象已过渡到 S3 Glacier Deep Archive 或云存储池使用 Azure Blob Storage，则无法使用 `Expedited`层还原该对象。将返回以下错误 `403 Forbidden`, `InvalidTier`： `Retrieval option is not supported by this storage class`。
