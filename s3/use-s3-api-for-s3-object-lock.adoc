...
NOTE: StorageGRID 不支持 `x-amz-sdk-checksum-algorithm`请求标头。
...
要为存储桶启用 S3 Object Lock，请使用以下方法之一：
...
* 从租户管理器管理存储桶设置。请参阅 link:../tenant/update-default-retention-settings.html["更新S3对象锁定默认保留"]。
...
当新对象版本被摄取到存储桶时，如果未指定 `x-amz-object-lock-mode`和 `x-amz-object-lock-retain-until-date`，则应用默认保留模式。如果未指定 `x-amz-object-lock-retain-until-date`，则使用默认保留期限来计算保留截止日期。

如果在接收对象版本后修改了默认保留期限，则对象版本的 retain-until-date 保持不变，并且不会使用新的默认保留期限重新计算。

要完成此操作、您必须 `s3:PutBucketObjectLockConfiguration`具有权限或帐户root。

必须在 PutObjectLockConfiguration 请求中指定以下请求标头之一：
...
NOTE: StorageGRID 不验证此操作的 x-amz-checksum-* 标头的内容。



==== 请求示例

此示例为存储桶启用 S3 对象锁定（如果网格管理员已授予权限），并将默认保留模式设置为 COMPLIANCE，将默认保留期限设置为 6 年。
...
当新对象版本被摄取到存储桶时，如果未指定 `x-amz-object-lock-mode`，则应用默认保留模式。如果未指定 `x-amz-object-lock-retain-until-date`，则使用默认保留期限来计算保留截止日期。
...
* 如果 PutObject 请求中存在任何 x-amz-object-lock-* 请求标头，则必须包含以下请求标头之一：
+
[listing]
----
Content-MD5
...
----
+
这些标头不是 CopyObject 或 CreateMultipartUpload 所必需的。

* 如果存储桶未启用 S3 对象锁定并存在 x-amz-object-lock-* 请求标头，则返回 400 Bad Request (InvalidRequest) 错误。
...
NOTE: StorageGRID 不验证这些操作的 x-amz-checksum-* 标头的内容。
...
+
The `x-amz-bypass-governance-retention` 标头对没有 S3 Object Lock 的对象无效。
