...
* 多部分上传的对象是否包含在 link:../tenant/what-is-branch-bucket.html["分支桶"] 中取决于初始化摄取的时间和分支存储桶的时间筛选器：
+
** *Before*。如果提取是在分支存储桶的 _Before_ 时间戳之前启动的，则无论上传何时完成，都会包含多部分上传的对象。
** *After*。如果提取是在分支桶的 _After_ 时间戳之后启动的，则包括多部分上传的对象。
** *Between*。如果提取是在分支存储桶的 _From_ 和 _To_ 时间戳之间启动的，则无论上传何时完成，都会包含多部分上传的对象。
** *排除*。如果提取是在分支桶的 _Exclude from_ 时间之前或 _Exclude to_ 时间之后启动的，则会包含多部分上传的对象。
