...
---
= 使用 S3 UploadPartCopy 操作在 StorageGRID 中上传对象的一部分
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
StorageGRID 会自动检测在 CreateMultipartUpload 请求期间是否指定了校验和，并在 UploadPartCopy 操作中使用此校验和。您无需在 UploadPartCopy 请求中指定校验和。如果 CreateMultipartUpload 请求未指定校验和，StorageGRID 不会计算对象的默认 CRC-64/NVME 校验和。当 CreateMultipartUpload 未指定校验和时，StorageGRID 不包括复制部件的校验和。
