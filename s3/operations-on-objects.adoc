...
---
= StorageGRID 如何实现对象的 S3 REST API 操作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
*  `If-Match`和 `If-None-Match`标头的行为因您的 StorageGRID 配置而异。提供这些标头可能会返回错误消息（501 Not Implemented）。如果 `If-Match`或 `If-None-Match`标头被接受但不起作用，则可能会触发警报，其中包含有关启用更严格的 501 Not Implemented 错误行为的说明。
