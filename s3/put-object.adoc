...
* `Content-Encoding`
+
当您为 `aws-chunked`指定 `Content-Encoding`时，StorageGRID 不会验证以下项目：

+
** StorageGRID 不会根据分块数据验证  `chunk-signature`。
** StorageGRID 不会针对对象验证您为 `x-amz-decoded-content-length`提供的值。
...
NOTE: 如果 PutObject 请求未指定校验和，StorageGRID 不会计算对象的默认 CRC-64/NVME 校验和。
