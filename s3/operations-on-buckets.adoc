...
summary: StorageGRID 系统为每个 S3 租户帐户最多支持 20,000 个存储桶。 
---
...
[role="lead"]
StorageGRID 系统为每个 S3 租户帐户最多支持 20,000 个存储桶。

每个网格最多可以包含100、000个分段。

如果多个租户每个都有多达 5,000 个存储桶，则网格中的每个存储节点必须至少有 64 GB 的 RAM。

存储桶名称限制遵循 AWS US Standard 区域限制，但应进一步将其限制为 DNS 命名约定，以支持 S3 虚拟托管风格的请求。
...
您可以检查是否为单个存储区启用或禁用了对上次访问时间的更新。请参阅 link:get-bucket-last-access-time-request.html["获取存储分段上次访问时间"]。
...
+
*注意*：如果您的 CreateBucket 请求使用的区域尚未在 StorageGRID 中定义，则会发生错误。

* 您可以包含 `x-amz-bucket-object-lock-enabled`请求标头以创建启用了 S3 Object Lock 的存储桶。请参阅link:../s3/use-s3-api-for-s3-object-lock.html["使用S3 REST API配置S3对象锁定"]。
+
S3 Object Lock 需要存储桶版本控制，在创建启用了 S3 Object Lock 的存储桶时会自动启用。

+
您可以在创建存储桶时或稍后启用 S3 Object Lock。
...
DeleteBucketLifecycle
 a| 
从存储桶中删除生命周期配置。请参阅link:create-s3-lifecycle-configuration.html["创建 S3 生命周期配置"]。
...
*注意*：如果为此存储桶设置了非默认 ILM 策略标记，则将存在一个分配了值的 `NTAP-SG-ILM-BUCKET-TAG`存储桶标记。如果有 `NTAP-SG-ILM-BUCKET-TAG`存储桶标记，请勿发出 DeleteBucketTagging 请求。相反，仅使用 `NTAP-SG-ILM-BUCKET-TAG`标记及其分配的值发出 PutBucketTagging 请求，以从存储桶中删除所有其他标记。请勿修改或删除 `NTAP-SG-ILM-BUCKET-TAG`存储桶标记。
...
(以前称为GET分段生命周期)
 a| 
返回存储区的生命周期配置。请参阅 link:create-s3-lifecycle-configuration.html["创建 S3 生命周期配置"]。
...
GetBucketLocation
 a| 
返回使用  `LocationConstraint` 元素在 CreateBucket 请求中设置的区域。如果存储桶的区域为  `us-east-1`（默认），则会为  `LocationConstraint` 返回空值。
...
请参阅link:create-s3-lifecycle-configuration.html["创建 S3 生命周期配置"]。要了解存储桶生命周期中的到期操作如何与 ILM 放置说明交互，请参阅link:../ilm/how-ilm-operates-throughout-objects-life.html["ILM 如何在对象的整个生命周期内运行"]。
...
PutBucketPolicy
 a| 
设置附加到存储桶的策略。请参阅 link:use-access-policies.html["使用存储分段和组访问策略"]。
...
* StorageGRID 仅支持复制配置的 V1。这意味着 StorageGRID 不支持对规则使用 `Filter`元素，并遵循 V1 约定删除对象版本。有关详细信息，请参阅 https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication-add-config.html["《Amazon Simple Storage Service用户指南：复制配置》"^]。
* 分段复制可以在分版本或未分版本的分段上配置。
* 您可以在复制配置 XML 的每个规则中指定不同的目标存储分段。一个源存储分段可以复制到多个目标存储分段。
* 目标存储桶必须指定为租户管理器或租户管理 API 中指定的 StorageGRID 终结点的 URN。请参阅 link:../tenant/configuring-cloudmirror-replication.html["配置 CloudMirror 复制"]。
+
...
* 如果网格管理员已授予权限，则在现有存储桶上启用 S3 对象锁定。


如果修改了默认保留期限，现有对象版本的 retain-until-date 将保持不变，并且不会使用新的默认保留期限重新计算。

有关详细信息，请参见 link:../s3/use-s3-api-for-s3-object-lock.html["使用S3 REST API配置S3对象锁定"]。
