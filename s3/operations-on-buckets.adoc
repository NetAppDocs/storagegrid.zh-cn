...
ListObjects (GET Bucket)和ListObjectVersies (GET Bucket)对象版本)操作支持StorageGRIDlink:consistency.html["一致性值"]。
...
* StorageGRID支持将Amazon Simple Notification Service (Amazon SNS)主题、Kafka主题或webhook端点作为目标。不支持简单队列服务(Simple Queue Service、SQS)或AWS Lamba端点。
