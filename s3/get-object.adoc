...
---
= 使用 GetObject 请求从 StorageGRID 中的存储桶检索对象
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* `Accept-Encoding`
+
当对象没有存储的 `Content-Encoding`时，带有 `Accept-Encoding: gzip`的 GET 请求会对响应实时应用压缩。只有 `gzip`会影响 GET 响应编码； `deflate`、 `br`和 `identity`不会影响。对于已存储 `Content-Encoding`的对象，StorageGRID 返回存储的编码，不会基于 `Accept-Encoding`应用额外的实时压缩。StorageGRID 不支持带有 `Accept-Encoding: gzip`的 GET 请求的范围读取操作。
