...
summary: 如果您的租户帐户在创建时分配了 Use grid federation connection 权限，则可以使用跨网格复制在租户源网格上的存储桶和租户目标网格上的存储桶之间自动复制对象。跨网格复制可以在一个方向或两个方向上发生。 
---
= 在 StorageGRID 中管理跨网格复制
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:grid-federation-cgr-workflow.png["跨网格复制工作流"]
...
+
image:grid-federation-cross-grid-replication-client-deletes.png["显示在两个网格上复制客户端删除的图像"]
