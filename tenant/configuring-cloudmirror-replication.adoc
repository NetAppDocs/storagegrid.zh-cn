...
---
= 在 StorageGRID 中配置 CloudMirror 复制
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 只有为 Amazon S3 或 StorageGRID S3 存储桶启用了 S3 Object Lock 的源或目标存储桶才支持复制。对象锁标头未复制到目标对象。
* 如果在包含对象的存储分段上启用CloudMirror复制、则会复制添加到该存储分段的新对象、但不会复制该存储分段中的现有对象。您必须更新现有对象才能触发复制。
* 如果在配置 CloudMirror 复制后在目标存储桶上启用 S3 对象锁定，则可能需要 link:../tenant/editing-platform-services-endpoint.html["重新保存端点"] 以允许复制恢复。
