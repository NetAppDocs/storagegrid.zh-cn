...
summary: '如果创建或编辑租户以使用网格联合连接、则会将该租户从一个StorageGRID系统(源租户)复制到另一个StorageGRID系统(副本租户)。复制租户后、添加到源租户的任何组和用户都会克隆到副本租户。' 
---
...
image:grid-federation-account-clone-workflow-tm.png["租户的帐户克隆工作流。下面将介绍这些步骤。"]
...
image:grid-federation-account-clone.png["显示本地组已从源网格克隆到目标网格的图像"]
...
image:grid-federation-local-user-clone.png["显示本地用户已从源网格克隆到目标网格的图像"]
...
image:grid-federation-federated-group-clone.png["显示联盟组已从源网格克隆到目标网格的图像"]
...
image:grid-federation-s3-access-key.png["显示S3访问密钥可以选择从源网格克隆到目标网格的图像"]
...
image:grid-federation-account-not-cloned.png["显示目标网格上的详细信息未克隆到源网格的图像"]
...
image:grid-federation-account-clone-edit-delete.png["显示编辑或删除的详细信息不会克隆的图像"]
