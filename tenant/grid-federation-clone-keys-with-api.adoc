...
summary: 如果您的 StorageGRID 租户帐户具有使用网格联合连接权限，则可以使用租户管理 API 手动克隆 S3 访问密钥。 
---
...
+
image:grid-federation-post-current-user-replicate.png["发布端点以复制自己的访问密钥"]
...
+
image:grid-federation-clone-access-key.png["租户管理器API、输入用于克隆访问密钥的值"]
...
+
image:grid-federation-post-other-user.png["发布用于复制其他用户访问密钥的端点"]
