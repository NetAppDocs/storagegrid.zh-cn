...
* 您正在将数据保存在版本化的存储桶中。存在一个意外漏洞，导致许多不需要的对象在 _T1_ 和 _T2_ 之间被摄取。您可以为 _Exclude_ 时间值 _T1_ 和 _T2_ 创建分支存储桶，并将客户端操作重定向到该分支存储桶。然后，只有在 _T1_ 之前和 _T2_ 之后摄取的对象才会向客户端公开。
...
另请参阅link:../ilm/how-objects-and-delete-markers-are-deleted.html#delete-s3-versioned-objects["如何删除受版本控制的 S3 对象"]。
