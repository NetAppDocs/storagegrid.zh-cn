...
在 StorageGRID 12.1 升级期间：

* 所有 StorageGRID 设备节点都会自动升级到 StorageGRID Appliance Installer 固件版本 12.1。
* SG6060 和 SGF6024 设备自动升级到 BIOS 固件版本 3B08.EX 和BMC固件版本 4.01.07。
* SG100 和 SG1000 设备自动升级到 BIOS 固件版本 3B13.EC 和BMC固件版本 4.75.07。
* SGF6112、SG6160、SG110 和 SG1100 设备自动升级为 BIOS 固件版本 3A14.QD 和 BMC 固件版本 3.21.07。
...
例如，当服务停止时，您可能会看到*无法与节点通信*警报，或者当某些节点已升级到 StorageGRID 12.0 或更高版本，但其他节点仍在运行较早版本的 StorageGRID 时，您可能会看到 *Cassandra 通信错误*警报。通常，这些警报将在升级完成后清除。

在升级到 StorageGRID 12.0 或更高版本期间停止存储节点时，可能会触发 *ILM placement unachievable* 警报。升级完成后，此警报可能会持续 1 天。
...
NOTE: 配置更改限制是特定于版本的。如果要升级到其他 StorageGRID 版本，请参见该版本升级说明中的受限更改列表。
...
* 升级时，不应管理 HA 组、VLAN 接口或负载平衡器端点。
* 在升级完成之前，请勿删除任何 HA 组。其他 HA 组中的虚拟 IP 地址可能无法访问。
