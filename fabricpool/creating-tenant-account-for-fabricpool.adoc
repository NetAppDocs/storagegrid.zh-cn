...
. 在“定义root访问权限”步骤中，根据您的StorageGRID系统是使用link:../admin/using-identity-federation.html["身份联合"]、，link:../admin/how-sso-works.html["单点登录(SSO)"]还是同时使用这两者，指定哪个用户将拥有租户帐户的初始root访问权限。
+
[cols="1a,2a"]
|===
| 选项 | 执行此操作 ... 
