...
---
= 访问并完成 StorageGRID FabricPool 设置向导
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. 如果仪表板上出现 *FabricPool and S3 setup wizard* 横幅，请选择横幅中的链接。如果横幅不再出现，请从 Grid Manager 标题栏中选择帮助图标 image:icon_help_menu_bar.png["帮助图标"]，然后选择 *FabricPool and S3 setup wizard*。
...
. 根据您的StorageGRID系统是否使用，定义租户帐户的根访问权限link:../admin/using-identity-federation.html["身份联合"]，link:../admin/how-sso-works.html["单点登录(SSO)"] ，或两者兼而有之。
+
[cols="1a,3a"]
|===
| 选项 | 执行此操作 ... 
...
.步骤
. （可选）选择复制图标（image:icon_tenant_copy_url.png["复制图标"]），将访问密钥 ID 和秘密访问密钥复制到剪贴板。
+
