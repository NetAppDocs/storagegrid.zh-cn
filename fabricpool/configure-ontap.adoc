...
---
= 配置 ONTAP System Manager 以将 StorageGRID 添加为 FabricPool 云层
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
URL模式
 a| 
如果是link:../admin/configuring-s3-api-endpoint-domain-names.html["已配置S3端点域名"]，请选择*Virtual-Hosted-Style URL*。

*路径样式 URL* 是 ONTAP 的默认设置，但建议对 StorageGRID 使用虚拟托管样式请求。如果为*服务器名称 (FQDN)* 字段提供的是 IP 地址而非域名，则必须使用*路径样式 URL*。
