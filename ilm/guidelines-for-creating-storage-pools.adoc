...
+
吞吐量的降低会影响对象摄取和检索的最大可实现速率（当选择 Balanced 或 Strict 作为摄取行为时），或可能导致 ILM 队列积压（当选择 Dual commit 作为摄取行为时）。请参阅 link:what-ilm-rule-is.html#ilm-rule-ingest-behavior["ILM规则加热行为"]。
