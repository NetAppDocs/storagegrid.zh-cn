...
image:ilm_replication_at_2_sites.png["在2个站点分别复制1个副本"]

image:ilm_replication_make_2_copies_2_pools_2_sites.png["在2个站点(站点专用存储池)中的每个站点创建1个副本"]
...
image:ilm_erasure_coding_site_loss_protection_4+2.png["EC 1存储池9存储节点3个站点"]
...
image:ec_three_sites_4_plus_2_site_loss_example_template.png["通过使用4+2 EC方案对3个站点进行纠删编码来存储对象"]

如果一个站点丢失、数据仍可恢复：

image:ec_three_sites_4_plus_2_site_loss_example.png["纠删编码4+2 EC方案1站点丢失"]
