...
* 一般来说，云存档存储（例如 Amazon S3 Glacier 或 Azure Blob Storage）是存储对象数据的低成本场所。但是，从云存档存储中检索数据的成本相对较高。为了实现最低的总体成本，您必须考虑何时以及多久访问一次云存储池中的对象。建议仅对您希望不经常访问的内容使用云存储池。
...
不要对与 Cloud Storage Pool 一起使用的容器使用 Azure Blob Storage 生命周期管理。生命周期操作可能会干扰 Cloud Storage Pool 操作。
