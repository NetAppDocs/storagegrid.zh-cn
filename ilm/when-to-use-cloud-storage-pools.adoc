...
如果要将对象数据分层或备份到多个云，则可以配置多个云存储池端点。ILM 规则中的筛选器允许您指定存储在每个云存储池中的对象。例如，您可能希望将来自某些租户或存储桶的对象存储在 Amazon S3 Glacier 中，并将来自其他租户或存储桶的对象存储在 Azure Blob Storage 中。或者，您可能希望在 Amazon S3 Glacier 和 Azure Blob Storage 之间移动数据。
