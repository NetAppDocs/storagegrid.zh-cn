...
* 当您开始将此存储桶用于云存储池后，为 Amazon S3 存储桶启用具有默认保留的 S3 对象锁定时。当 PUT 操作没有包含有效负载校验和值的 HTTP 标头（例如 `Content-MD5`）时，会出现此错误。AWS 要求将此标头值用于对启用了 S3 对象锁定的存储桶执行的 PUT 操作。
+
要解决此问题，请按照 link:../ilm/editing-cloud-storage-pool.html["编辑云存储池"] 中的步骤操作，无需进行任何更改。此操作会触发对云存储池配置的验证，该配置会自动检测和更新云存储池端点配置上的 S3 对象锁定标志。

* 当您开始将此存储桶用于云存储池后，为 StorageGRID S3 存储桶启用具有默认保留的 S3 对象锁定时。
+
要解决此问题：

+
** 如果您使用的是 HTTP：不支持启用 S3 对象锁定的云存储池。请编辑端点连接以改用 HTTPS。
** 如果您使用的是 HTTPS：请按照 link:../ilm/editing-cloud-storage-pool.html["编辑云存储池"] 中的步骤操作，无需进行任何更改。此操作将触发对云存储池配置的验证，该验证会自动检测并更新云存储池端点配置上的 S3 Object Lock 标志。
