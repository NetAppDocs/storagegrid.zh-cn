...
summary: 云存储池指定单个外部 Amazon S3 存储桶或其他 S3 兼容提供商或 Azure Blob Storage 容器。 
---
...
[role="lead"]
云存储池指定单个外部 Amazon S3 存储桶或其他 S3 兼容提供商或 Azure Blob Storage 容器。

创建云存储池时，您需要指定 StorageGRID 用于存储对象的外部存储分段或容器的名称和位置、云提供商类型（Amazon S3/GCP 或 Azure Blob Storage），以及 StorageGRID 访问该外部存储分段或容器所需的信息。
