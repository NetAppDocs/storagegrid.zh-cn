...
== 允许在现有存储桶上启用 S3 对象锁定的要求

启用全局 S3 对象锁定设置后，您可以选择允许所有具有权限的 S3 租户帐户或 S3 用户在现有存储桶上启用 S3 对象锁定。当 S3 租户帐户或 S3 用户在现有存储桶上启用 S3 对象锁定时，可能会导致意外的数据从云存储池迁移回 StorageGRID，具体取决于您的 ILM 规则。


CAUTION: 意外数据迁移启动后无法停止。
