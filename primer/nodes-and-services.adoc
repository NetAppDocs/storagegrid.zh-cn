...
管理节点:: 提供系统配置、监控、日志等管理服务。当您登录到 Grid Manager 时，您正在连接到管理节点。每个网格必须有一个主管理节点，并且可能有其他非主管理节点以实现冗余。您可以连接到任何管理节点，每个管理节点都会显示 StorageGRID 系统的类似视图。但是，您必须使用主管理节点执行大多数维护过程。
+
--
...
+
--
在新存储节点的初始安装过程中，您可以指定它仅用于 link:../primer/what-storage-node-is.html#types-of-storage-nodes["存储元数据或数据"]。
...
* https://docs.netapp.com/us-en/storagegrid-appliances/["StorageGRID 设备文档"^]
...
gdu-server
 a| 
用于网格维护和配置操作的后端任务运行程序。接受来自 `mgmt-api`和其他实用程序脚本的请求，对其进行验证，并将每个操作发送到相应的 GDU 任务处理程序，例如设置、扩展、停用和升级。
 a| 
管理节点
...
高可用性
 a| 
管理在"高可用性组"页面上配置的节点上的高可用性虚拟 IP。此服务也称为 _keepalived_ 服务。
 a| 
管理节点和网关节点
...
mgmt-api
 a| 
网格管理器后端 API。公开用于管理网格（配置、监控和维护工作流程）的经过身份验证的 REST 端点，强制执行验证和权限，并协调长期运行的操作。网格管理器界面和自动化客户端使用的主控制平面 API。
 a| 
管理节点
