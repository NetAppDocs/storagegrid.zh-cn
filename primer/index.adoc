...
image:storagegrid_system_diagram.png["具有多个客户端的映像"]
...
* Amazon Web Services 简单存储服务 (S3) 标准对象存储协议。
...
* 联合身份管理。与 Active Directory、OpenLDAP 或 Oracle Directory Service 集成以进行用户身份验证。支持使用安全断言标记语言 2.0 (SAML 2.0) 或 OpenID Connect (OIDC) 的单点登录 (SSO)，以在 StorageGRID 与外部身份提供程序（例如 Active Directory Federation Services (AD FS)）之间交换身份验证和授权数据。
