...
* stream-access.log：流块的 TCP/TLS 流（第 4 层）代理会话。
+
*注意*：这不是 HTTP 请求日志。
...
路由的网格联合流量的访问日志，类似于 S3（全局命名空间/联合 URL）。
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log`：系统存储桶日志。
...
`update-cors-configurations.log`：跟踪与 TLS/cipher 和 CORS 更改相关的日志。
