...
[role="lead"]
根据需要，配置审核级别、协议标头、审核消息和日志的位置以及日志保留。

所有StorageGRID节点都会生成审计消息和日志来跟踪系统活动和事件。审计消息和日志是监控和故障排除的重要工具。

您也可以link:../monitor/considerations-for-external-syslog-server.html["配置外部系统日志服务器"]选择远程保存审核信息。通过使用外部服务器，您可以最大限度地减少审核消息日志记录对性能的影响，同时不会降低审核数据的完整性。如果您拥有大型网格、使用多种类型的 S3 应用程序或希望保留所有审核数据，则外部 syslog 服务器特别有用。
...
+
当 StorageGRID 在请求中找到 HTTP 标头时，它会将其包含在 HTRH 字段下的审核消息中。
...
默认情况下，审核消息和日志保存在生成它们的节点上。StorageGRID 定期轮换并最终删除审核消息和日志，以防止它们占用过多的磁盘空间。要在外部保存审核消息和一部分日志， <<use-external-syslog-server,使用外部系统日志服务器>>。
...
+
如果选择一个值，StorageGRID 会将所选值应用于此类型的所有消息。如果使用固定值覆盖严重性，则会丢失有关不同严重性的详细信息。
...
在使用外部 syslog 服务器之前，请要求网格中的所有节点向外部 syslog 服务器发送测试消息。这些测试消息可帮助您在将数据发送到外部 syslog 服务器之前验证整个日志收集基础架构。
...
您可以通过创建 ILM 规则或配置 S3 存储桶生命周期策略来管理 StorageGRID 保留日志的时长。

[role="tabbed-block"]
====
.ILM规则
--
为存储桶创建 ILM 规则，并将此规则添加到活动 ILM 策略。

.步骤
. 选择 * ILM * > * 规则 * 。
. 选择 *Create*。link:../ilm/create-ilm-rule-enter-details.html["第1步(输入详细信息)"] 此时将显示 Create an ILM rule wizard（创建 ILM 规则向导）：
+
.. 请输入规则的唯一名称。例如：归档日志的 ILM 规则。
.. 选择日志存档 S3 租户帐户。
.. 指定日志存档 S3 存储桶。


. 继续link:../ilm/create-ilm-rule-define-placements.html["第2步(定义放置位置)"]并定义存档日志规则的放置位置。例如，ILM 规则在第一年内在站点 1 中存储一个复制副本，在站点 2 中存储一个复制副本。一年后，仅在一个站点上创建并保存 2+1 纠删码副本。
. 创建规则后，将其添加到活动的 ILM 策略。


--
.S3 bucket 生命周期
--
在日志存档 S3 存储桶上配置 S3 存储桶生命周期策略：
...
----
在日志存档 S3 存储桶上配置 S3 存储桶生命周期策略时，生命周期到期操作将覆盖与存储桶生命周期筛选器匹配的对象的 ILM 策略。与存储桶生命周期筛选器不匹配的对象使用 ILM 策略的保留设置。如果对象与存储桶生命周期筛选器匹配，且未明确指定过期操作，则不会使用 ILM 策略的保留设置，这意味着对象版本将被永久保留。
