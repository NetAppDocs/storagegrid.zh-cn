...
* image:icon_alert_green_checkmark.png["图标警报绿色复选标记"] *Normal*：所有值都在正常范围内。
* image:icon_alert_yellow_minor.png["图标警报黄色次要"] *Attention*：一个或多个值超出正常范围。
* image:icon_alert_red_critical.png["图标警报红色严重"] *Caution*：一个或多个值明显超出正常范围。
...
+
image:support_diagnostics_page.png["支持诊断页面"]

. 要了解有关特定诊断的更多信息，请选择该行。
+
...
+
image:support_diagnostics_cpu_utilization.png["诊断状态值示例"]
...
+
image:grafana_dashboard_nodes.png["Grafana 信息板节点"]

. *可选*：要查看 Prometheus 表达式随时间变化的图表，请选择 *View in Prometheus*。
+
此时将显示诊断中使用的表达式的 Prometheus 图形。

+
image:support_diagnostics_prometheus_png.png["支持诊断程序页面"]
