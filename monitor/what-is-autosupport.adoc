...
使用AutoSupport有助于更快地解决问题。技术支持可以监控您系统的存储需求并帮助确定是否需要添加新节点或站点。或者， AutoSupport可以将软件包发送到另一个目的地。
...
NOTE: 如果未启用StorageGRID AutoSupport ，则网格管理器仪表板上会出现一条消息。该消息包含指向AutoSupport配置页面的链接。如果您关闭该消息，即使AutoSupport保持禁用状态，它也不会再次出现，直到您的浏览器缓存被清除。
...
Active IQ Digital Advisor（也称为 Digital Advisor）是基于云的，利用了 NetApp 已安装基础中的预测分析和社区智慧。其持续的风险评估、预测性警报、规范性指导和自动化操作可帮助您在问题发生之前进行预防，从而改善系统健康状况并提高系统可用性。
