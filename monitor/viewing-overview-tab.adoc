...
image:nodes_page_overview_tab.png["节点页面概述选项卡"]
...
+
** *未知* image:icon_alarm_blue_unknown.png["蓝色问号图标"]：由于未知原因，节点未连接到网格，或者一个或多个服务意外关闭。例如，节点之间的网络连接已丢失、电源已关闭或服务已关闭。*无法与节点通信*警报也可能被触发。其他警报也可能处于活动状态。这种情况需要立即关注。
+

NOTE: 在受管关闭操作期间，节点可能会显示为未知。在这些情况下，您可以忽略未知状态。

** *管理性关闭* image:icon_alarm_gray_administratively_down.png["灰色问号图标"]：由于预期的原因，节点未连接到网格。例如，节点或节点上的服务已正常关闭、节点正在重新启动或软件正在升级。一个或多个警报也可能处于活动状态。
** *已连接* image:icon_alert_green_checkmark.png["图标警报绿色复选标记"]：节点已连接到网格。
...
* *IP addresses*：节点的 IP 地址。选择 *Show additional IP addresses* 以查看节点的 IPv4 和 IPv6 地址以及接口映射。
...
image:nodes_page_alerts_table.png["节点页面警报表"]
