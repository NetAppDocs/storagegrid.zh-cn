...
+
image:dashboard_data_and_metadata_space_usage_breakdown.png["数据和元数据空间使用情况明细"]

.. 记下随时间变化的存储卡上的图表。使用时间段下拉列表帮助您确定存储的使用速度。
+
image:dashboard_storage_over_time.png["存储随时间变化图表"]
...
.. 选择 * ； grid_* > * 存储 * 。
+
image:nodes_deployment_storage_tab.png["节点部署存储选项卡"]
...
image:calculating_watermarks.gif["可用总空间的概念图"]
...
+
image:nodes_page_storage_used_object_data.png["节点页面存储已用对象数据"]
...
NOTE: 要查看这些值的图形，请单击"可用列"中的图表图标 image:icon_chart_new_for_11_5.png["图表图标"]。

+
image:nodes_page_storage_tables.png["对象存储表"]
...
+
image:storage_used_object_metadata.png["已用存储—对象元数据"]
...
image:forecast-metadata-usage.png["按站点预测元数据使用量"]
