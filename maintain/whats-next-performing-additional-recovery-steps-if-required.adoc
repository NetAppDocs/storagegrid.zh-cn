...
* link:selecting-start-recovery-to-configure-non-primary-admin-node.html["使用非主管理节点恢复主管理节点"]
...
* 如果对于任何 `<PURPOSE>`，配置文件变量的值 `BLOCK_DEVICE_<PURPOSE>`引用的块设备所包含的数据与主机发生故障前的数据不同。
 ...
 a| 
link:reviewing-warnings-about-storage-volume-recovery.html["从系统驱动器完好无损的存储卷故障中恢复"]
