...
---
= 恢复发生故障的存储卷并在 StorageGRID 中重建 Cassandra 数据库
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.下一步是什么？
在重新格式化和重新装载存储卷以及必要的 Cassandra 操作完成后，您可以 link:../maintain/restoring-object-data-to-storage-volume-where-system-drive-is-intact.html["使用网格管理器还原对象数据"]。
