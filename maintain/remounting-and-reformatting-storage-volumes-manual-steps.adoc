...
---
= 重新挂载并重新格式化 StorageGRID 存储卷（手动步骤）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
在存储节点恢复期间，如果 Grid Manager 中出现"等待手动步骤"消息，请按照以下步骤重新挂载并重新格式化存储卷。如果您没有看到此消息，则重新挂载和重新格式化操作已自动完成，您可以跳过_this_过程和link:../maintain/restoring-object-data-to-storage-volume.html["还原对象数据"]。
...
+
** 在大多数情况下，您应该link:../maintain/restoring-object-data-to-storage-volume.html["使用网格管理器还原对象数据"]。答案 `y`以使用 Grid Manager。
