...
summary: 您可以使用更改 IP 工具更改一个或多个节点的网络配置。您可以更改网格网络的配置，也可以添加、更改或删除管理员或客户端网络。您还可以更改设备节点 BMC 的 LAN IP 设置。 
---
= 更改 StorageGRID 中的节点网络配置
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您可以使用更改 IP 工具更改一个或多个节点的网络配置。您可以更改网格网络的配置，也可以添加、更改或删除管理员或客户端网络。您还可以更改设备节点 BMC 的 LAN IP 设置。
...
NOTE: 如果您的 StorageGRID 系统启用了单点登录 (SSO)，并且您正在更改管理节点的 IP 地址，请注意，使用管理节点的 IP 地址（而不是建议的完全限定域名）配置的任何依赖方信任都将失效。您将无法再登录到节点。更改 IP 地址后，必须立即使用新 IP 地址更新或重新配置节点在 Active Directory 联合身份验证服务 (AD FS) 中的依赖方信任。请参阅使用 link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] 或 link:../admin/learn-about-setting-up-sso-saml.html["SAML"] 设置 SSO 的说明。
...
+
image:change_ip_tool_main_menu.png["显示更改 IP 工具欢迎屏幕的屏幕截图"]
...
+
image:change_ip_tool_edit_ip_mask_sample_output.png["周围文本所述的屏幕截图"]
...
+
image:change_ip_tool_validate_sample_error_messages.gif["周围文本所述的屏幕截图"]

+
在此示例中，验证已通过。

+
image:change_ip_tool_validate_sample_passed_messages.gif["周围文本所述的屏幕截图"]
...
== 更改设备节点 BMC LAN IP 设置

您可以更改设备节点 BMC 的 LAN IP 设置。

. 在 Grid Manager 中，转到 *节点* > *_网格节点_* > *BMC 设置*。
. 在 LAN IP 设置部分，选择 *静态* 或 *DHCP* 的 IP 分配类型。
. 输入 BMC 的 MAC 地址、IPv4 地址 (CIDR) 和默认网关。
. 选择 * 保存 * 。


您还可以使用 `PUT /bmc-configuration/root-password`或 `GET /bmc-configuration/root-password` API 更改或查看 BMC LAN IP 设置。
