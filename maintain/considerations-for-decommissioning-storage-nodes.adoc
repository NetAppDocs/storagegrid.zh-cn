...
* 系统必须始终包括足够的存储节点，以满足操作要求，包括link:understanding-adc-service-quorum.html["ADC仲裁"]和活动的link:reviewing-ilm-policy-and-storage-configuration.html["ILM策略"]。如果停用会使站点的 ADC 计数低于法定数量，StorageGRID 会自动将 ADC 服务移动到同一站点上的可用非 ADC 存储节点，从而保持 ADC 法定数量。如果无法自动移动 ADC 服务，则可能需要先link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["在扩展操作中添加新 Storage Node"]，然后才能停用现有存储节点。
+
