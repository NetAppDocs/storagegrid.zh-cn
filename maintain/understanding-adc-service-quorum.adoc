...
summary: '如果停用后仍保留的管理域控制器(ADC)服务太少、则可能无法停用站点上的某些存储节点。' 
---
...
如果需要停用 Storage Node，可以通过link:../release-notes/changes-to-grid-management-api.html#new-private-endpoints-for-move-adc["将 ADC 服务移动到同一站点上的另一个存储节点"]或在link:../expand/index.html["扩展"]中添加新的 Storage Node 并指定其应具有 ADC 服务来继续满足 ADC 仲裁要求。然后，可以在不影响仲裁的情况下停用现有 Storage Node。
