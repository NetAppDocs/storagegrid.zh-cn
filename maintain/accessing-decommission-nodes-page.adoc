...
** 输入搜索词可快速查找特定节点。
+
在此示例中，"可能停用"列中的绿色复选标记表示可以停用 API Gateway Node 和纯数据 Storage Node。由于各种原因，其他节点无法停用。例如，无法停用主 Admin Node，因为不支持停用此节点类型。仅元数据 Storage Node 无法停用，因为该站点将不再满足 link:../maintain/understanding-adc-service-quorum.html["ADC仲裁"]。

+
image:decommission_nodes_page.png["停用节点页面"]
...
** image:icon_alarm_gray_administratively_down.png["灰色问号图标"] （灰色）：管理性关闭
** image:icon_alarm_blue_unknown.png["蓝色问号图标"] （蓝色）：未知
...
编辑 HA 组以删除节点的接口或删除整个 HA 组。请参阅。 link:../admin/configure-high-availability-group.html["配置高可用性组"]
 ...
 a| 
否，站点 _x_ 需要至少 3 个具有 ADC 服务的存储节点或可用的非 ADC 节点，才能将 ADC 服务移至其中。如果任何节点处于脱机状态，则 ADC 节点停用不可用。
 a| 
*仅存储节点*（组合或仅元数据存储节点）。如果站点中没有足够的节点来支持 ADC 仲裁要求，并且无法自动移动 ADC 服务，则无法停用存储节点。自动移动需要跨网格的两个以上 ADC 存储节点联机，以及同一站点上可用的非 ADC 存储节点。
 a| 
确保满足自动 ADC 服务移动的条件（请参阅link:../maintain/understanding-adc-service-quorum.html["ADC仲裁"]），或将其link:../maintain/move-adc-service.html["手动移动 ADC 服务"]移动到同一站点的其他存储节点。您也可以通过向站点添加新的存储节点并指定其应具有 ADC 服务来执行扩展。请参阅link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["向 StorageGRID 添加网格节点的工作流摘要"]。
