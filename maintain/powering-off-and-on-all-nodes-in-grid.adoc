...
当您关闭站点或网格中的所有节点时，存储节点脱机期间将无法访问已载入的对象。
...
. 如果您允许外部 SSH 访问，link:../admin/manage-external-ssh-access.html["阻止访问"]当您关闭完节点后。
...
NOTE: 如果主机包含多个网格节点，则当您打开主机时，这些节点会自动恢复联机状态。

.步骤
. 打开主管理节点和任何非主管理节点的主机的电源。
+

NOTE: 在重新启动存储节点之前，您无法登录到管理节点。
...
. 启动所有网关节点的主机。
. 登录到网格管理器。
. 选择*节点*并监控网格节点的状态。验证节点名称旁边没有警报图标。
