...
.关于此任务
您可以通过在*健康*列中查找蓝色未知图标image:icon_alarm_blue_unknown.png["图标未知"]或灰色管理关闭图标image:icon_alarm_gray_administratively_down.png["图标已被管理员关闭"]来识别断开连接的节点。
...
. 一旦新的恢复包可用，请选择链接或选择*维护* > *系统* > *恢复包*以访问恢复包页面。然后，下载 `.zip`文件。
+
...
.纠删编码(EC)数据
--
要监控擦除编码数据的修复并重试可能失败的任何请求，请选择 *Support* > *Tools* > *Metrics* 以查看当前作业的估计完成时间和完成百分比。然后，在 Grafana 部分选择 *EC Overview*。查看 *Grid EC Job Estimated Time to Completion* 和 *Grid EC Job Percentage Completed* 仪表板。
