...
* 您可以指定主管理节点的 IP 地址。如果主管理节点不可用，则指定非主管理节点的 IP 地址（如果可用）。您还可以确认节点的主机名（系统名称）。
* 您开始安装，并等待卷配置完毕并安装软件。
+
...
. 在*节点名称*字段中，输入用于正在恢复的节点的相同主机名（系统名称），然后选择*保存*。
. 在安装部分，确认当前状态为"准备好开始将 `_node name_`安装到具有主管理节点 `_admin_ip_` 的网格中"，并且*开始安装*按钮已启用。
+
...
NOTE: 如果需要手动访问监视器安装页面，请从菜单栏中选择*监视器安装*。请参阅 https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/monitoring-appliance-installation.html["监控设备安装"^]。
