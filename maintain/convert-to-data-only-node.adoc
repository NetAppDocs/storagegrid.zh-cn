...
* 存储节点不是仅元数据节点或仅数据节点。
* 如果存储节点包含 ADC 服务，则网格中有两个以上的 ADC 存储节点联机，并且同一站点上有可用于托管 ADC 服务的非 ADC 存储节点。
...
.关于此任务
使用此过程可将一个或多个组合的存储节点转换为仅限数据的节点。如果存储节点包含 ADC 服务，则在转换过程中，该服务将自动移动到同一站点上的另一个存储节点。
...
NOTE: StorageGRID 将 ADC 服务移动到满足托管服务要求的最新 Storage Node。
