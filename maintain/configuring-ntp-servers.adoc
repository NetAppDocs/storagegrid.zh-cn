...
summary: '您可以添加、更新或删除网络时间协议(NTP)服务器、以确保StorageGRID 系统中网格节点之间的数据准确同步。' 
---
= 管理 StorageGRID 中的 NTP 服务器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: 为生产级 StorageGRID 安装指定外部 NTP 源时，请勿在 Windows Server 2016 之前的 Windows 版本上使用 Windows Time (W32Time) 服务。早期版本的 Windows 上的时间服务不够准确，Microsoft 不支持在高精度环境中使用，包括 StorageGRID。有关详细信息，请参见 https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["高精度时间的支持边界"^]。
