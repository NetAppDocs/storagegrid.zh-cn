...
* 恢复软件包文件的最新备份(`sgws-recovery-package-_id-revision_.zip`)。
* 您具有配置密码短语。
...
+
.. 单击 * 浏览 * 。
.. 找到StorageGRID系统的最新恢复软件包文件，然后单击*Open*。
...
NOTE: 要配置依赖方信任，请参见link:../admin/configure-sso.html["配置单点登录"]。要访问默认服务器证书，请登录到管理节点的命令 Shell 。转到 `/var/local/mgmt-api`目录、然后选择 `server.crt`文件。
