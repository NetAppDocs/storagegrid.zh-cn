...
. 选择*恢复发生故障的主管理节点*。
+
image:install_or_recover_primary_admin_node.png["安装或恢复主管理节点"]

. 上传恢复包的最新备份：
+
.. 选择 * 浏览 * 。
.. 找到您的 StorageGRID 系统的最新恢复包文件，然后选择*打开*。
...
NOTE: 要配置依赖方信任，请参阅使用 link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] 或 link:../admin/learn-about-setting-up-sso-saml.html["SAML"] 设置 SSO。要访问默认服务器证书，请登录到管理节点的命令 shell。转到  `/var/local/mgmt-api` 目录，然后选择  `server.crt` 文件。
