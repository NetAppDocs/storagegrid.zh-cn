...
请联系技术支持。
 a| 
如果多个存储节点发生故障，则技术支持必须协助恢复，以防止可能导致数据丢失的 Cassandra 数据库不一致。可能需要站点恢复程序。

技术支持可以确定何时可以安全地开始恢复第二个 Storage Node。

*注意*：如果站点上包含 ADC 服务的多个存储节点发生故障，则该站点的任何待处理平台服务请求都将丢失。
