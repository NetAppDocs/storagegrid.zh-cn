...
如果从非主 Admin 节点恢复节点，请将 ADMIN_IP 设置为_非主 Admin 节点_的网格网络 IP 地址，以便恢复的节点向非主 Admin 节点而不是主 Admin 节点注册。要查看管理网络密钥的规则，请参阅 link:../swnodes/creating-node-configuration-files.html["为 Linux 部署创建 StorageGRID 节点配置文件"]。
