...
* StorageGRID 系统中的所有节点都必须已连接（image:icon_alert_green_checkmark.png["图标警报绿色复选标记"]）。
+
...
.步骤
. 查看步骤 5（解决节点冲突）的断开的节点部分，以确定 StorageGRID 系统中是否有任何节点的连接状态为未知（image:icon_alarm_blue_unknown.png["图标警报蓝色未知"]）或管理性关闭（image:icon_alarm_gray_administratively_down.png["图标警报灰色已管理员关闭"]）。
+
image:decommission_site_step_5_disconnected_nodes.png["停用站点步骤 5 解决节点冲突"]
...
+
image:decommission_site_step_5_ha_groups.png["停用站点步骤 5 HA 组"]
...
+
如果所有节点都已连接，并且所选站点中的节点未用于 HA 组，则启用 *Provisioning passphrase* 字段。
...
+
image:decommission_site_step_5_provision_passphrase.png["停用站点步骤 5 配置密码短语"]
...
+
image:decommission_site_step_5_warning.png["停用站点步骤 5 警告"]
...
+
image:decommission_site_step_5_grid_configuration.png["停用站点步骤 5 网格配置"]
