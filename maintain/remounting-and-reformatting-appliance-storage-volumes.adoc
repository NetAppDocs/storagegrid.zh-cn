...
summary: 如果自动过程无法重新挂载保留的存储卷并重新格式化发生故障的存储卷，则必须手动运行两个脚本来完成这些操作。第一个脚本重新挂载已正确格式化为 StorageGRID 存储卷的卷。第二个脚本重新格式化所有未挂载的卷，如果需要，重建 Cassandra 数据库，然后启动服务。 
---
...
[role="lead"]
如果自动过程无法重新挂载保留的存储卷并重新格式化发生故障的存储卷，则必须手动运行两个脚本来完成这些操作。第一个脚本重新挂载已正确格式化为 StorageGRID 存储卷的卷。第二个脚本重新格式化所有未挂载的卷，如果需要，重建 Cassandra 数据库，然后启动服务。

在存储节点恢复期间，如果 Grid Manager 中出现"等待手动步骤"消息，请按照以下步骤重新挂载并重新格式化存储卷。如果您没有看到此消息，则重新挂载和重新格式化操作已自动完成，您可以跳过本过程并link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["还原对象数据"]。
...
+
image:recovering_cassandra.png["显示网格管理界面中的恢复进度的屏幕截图"]

. 脚本在节点上启动服务后 `sn-recovery-postinstall.sh`、您可以将对象数据还原到由脚本格式化的任何存储卷。
+
该脚本询问您是否要使用 Grid Manager 卷还原过程来还原对象数据。

+
** 在大多数情况下，您应该link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["使用网格管理器还原对象数据"]。答案 `y`以使用 Grid Manager。
** 在极少数情况下，例如当您知道替换节点可用于对象存储的卷少于原始节点时，请联系技术支持以手动还原对象数据。如果其中一种情况适用，请回答 `n`并按任意键返回命令行。
