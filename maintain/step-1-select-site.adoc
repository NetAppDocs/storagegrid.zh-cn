...
+
image:decommission_site_step_select_site.png["停用站点步骤 1 选择站点"]
...
| 停用可能的原因 | 说明 | 下一步 
 ...
 a| 
绿色复选标记（image:icon_alert_green_checkmark.png["图标警报绿色复选标记"]）
 a| 
您可以停用此站点。
 a| 
...
不会。此站点上的所有节点均已断开连接。请联系您的 NetApp 客户代表。
 a| 
除非站点中的每个节点都已连接（image:icon_alert_green_checkmark.png["图标警报绿色复选标记"]），否则无法执行连接的站点停用。
 a| 
...
|===
+
示例显示了具有三个站点的 StorageGRID 系统。Raleigh 和 Sunnyvale 站点的绿色复选标记（image:icon_alert_green_checkmark.png["图标警报绿色复选标记"]）表示您可以停用这些站点。但是，您无法停用 Vancouver 站点，因为它包含主管理节点。
