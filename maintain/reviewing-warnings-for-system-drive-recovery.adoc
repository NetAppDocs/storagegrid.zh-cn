...
CAUTION: 如果此存储节点处于只读维护模式，以允许具有发生故障的存储卷的另一个存储节点检索对象，则在恢复此发生故障的存储节点之前，请恢复具有发生故障的存储卷的存储节点上的卷。请参阅 link:reviewing-warnings-about-storage-volume-recovery.html["从系统驱动器完好无损的存储卷故障中恢复"] 的说明。
