...
NOTE: 如果您的 StorageGRID 系统启用了 SSO，并且使用管理节点 IP 地址（而不是建议的完全限定域名）配置了任何依赖方信任，请准备好在更改 IP 地址后立即更新或重新配置 Active Directory Federation Services (AD FS) 中的这些依赖方信任。请参阅使用 link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] 或 link:../admin/learn-about-setting-up-sso-saml.html["SAML"] 设置 SSO。
