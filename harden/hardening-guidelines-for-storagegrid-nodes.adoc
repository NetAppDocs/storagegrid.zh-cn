...
您可以为所有包含 BMC 的设备启用或禁用远程 IPMI 访问。远程 IPMI 接口允许任何拥有 BMC 帐户和密码的人对您的 StorageGRID 设备进行低级硬件访问。如果您不需要远程 IPMI 访问 BMC，请禁用此选项。

* 要在 Grid Manager 中控制对 BMC 的远程 IPMI 访问，请转到 *Configuration* > *Security* > *Security settings* > *Appliances* > *Remote IPMI access*：
+
...
您可以更改设备的 BMC 密码，以限制访问并提高系统安全性。

. 在 Grid Manager 中，转到 *节点* > *_网格节点_* > *BMC 设置*。
. 在更改密码部分，在*新密码*中输入一个长度在 8 到 16 个字符之间的新 BMC 密码，然后在*确认新密码*中再次输入。
. 选择 * 保存 * 。


您还可以使用 `PUT /bmc-configuration/root-password` API 更改 BMC 密码。



[[control-remote-access-through-usb-portscontrol-external-usb-port-access-on-appliances]]
== [[control-remote-access-through-usb-ports]]控制设备上的外部 USB 端口访问

您可以启用设备上的 USB 端口，以允许技术支持访问以进行诊断和故障排除。如果您不需要 USB 端口访问，请禁用端口以确保安全。初始安装或升级后，默认情况下会启用 USB 端口。

* 在 Grid Manager 中，转到 *Configuration* > *Security* > *Security settings* > *Appliances* > *USB port access* 以启用或禁用 USB 端口：
+
** 选中*启用USB端口*复选框并保存以启用端口。
+
如果先前未选中USB复选框并已保存，请选中*启用USB端口*复选框并保存以再次启用USB端口。

** 清除*启用 USB 端口*复选框并保存以禁用端口。
...
+
您也可以 <<block-remote-ipmi,禁用远程 IPMI 访问>>。
