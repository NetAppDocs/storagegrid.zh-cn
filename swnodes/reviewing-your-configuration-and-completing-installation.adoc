...
. 查看 * 摘要 * 页面。
+
image:11_gmi_installer_summary_page.gif["网格管理器安装程序摘要页面"]

. 验证所有网格配置信息是否正确。使用摘要页面上的修改链接返回并更正任何错误。
. 选择*安装*。
+

NOTE: 如果节点配置为使用客户端网络，则当您选择*安装*时，该节点的默认网关将从网格网络切换到客户端网络。如果您失去连接，则必须确保您通过可访问的子网访问主管理节点。有关详细信息，请参阅link:../network/index.html["网络连接准则"]。
...
. 选择*我已成功下载并验证恢复包文件*复选框，然后选择*下一步*。
+
如果安装仍在进行中，则会显示状态页面。此页面指示每个网格节点的安装进度。

+
image:12_gmi_installer_status_page.gif["此图通过周围文字进行了说明。"]
